    ...
    if not dir_path.exists():
        ...
        for child in dir_path.iterdir():
            ...
            for idx, line in enumerate(content.splitlines()):
                params = line.split()
                if len(params) != 5:
                    raise ValueError(f"Неправильный формат в строке {idx} в файле {child}: {line}. Ожидается 5 параметров, получено {len(params)}")
                else:
                    try:
                    ...
                    for i in range(1, 5):
                        try:
                            ...
                            raise ValueError(f"Ошибка в файле {child} в строке {idx}: невозможно привести параметр '{params[i]}' в формат числа с плавающей точкой")
                        if param < 0 or param > 1:
                            raise ValueError(f"Ошибка в файле {child} в строке {idx}: параметр {param} находится вне диапазона [0, 1]")