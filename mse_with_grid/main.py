...
def run_training_task(run_id: str, search_alg: str, hyperparams: dict):
    try:
        append_to_run_log(run_id, "Подготовка гиперпараметров...")
        if search_alg == "GridSearch":
            hyperparams_combinations = grid_search_params(hyperparams)
        elif search_alg == "RandomSearch":
            hyperparams_combinations = random_search_params(hyperparams)
        else:
            hyperparams_combinations = []
        
        append_to_run_log(run_id, f"Сгенерировано {len(hyperparams_combinations)} комбинаций")
        ...
        append_to_run_log(run_id, "Начало обучения...")
        
        result = orc.run(hyperparams_combinations)
        ...
        if run_id in RUN_DETAILS:
            ...
            '''if result:
                RUN_DETAILS[run_id]["summary"]["bestModel"] = result.get("best_model")
                RUN_DETAILS[run_id]["summary"]["bestMap"] = result.get("best_map")
                RUN_DETAILS[run_id]["models"] = result.get("models", [])'''
        
            refresh_run_summary(run_id)
        
        #append_to_run_log(run_id, "Обучение завершено успешно!")
        
    except Exception as e:
        # В случае ошибки обновляем статус
        error_msg = f"Ошибка обучения: {str(e)}"
        print(error_msg)
        append_to_run_log(run_id, f"Ошибка: {error_msg}")
...
def append_to_run_log(run_id: str, message: str):
    """Добавляет сообщение в лог запуска"""
