...
def test_grid_search_params_base_case():
    assert grid_search_params({"lr0": [0.01, 0.00001], "test": ["test1", "test2"]}) == [{'lr0': 0.01, 'test': 'test1'},
                                                                                        {'lr0': 0.01, 'test': 'test2'},
                                                                                        {'lr0': 1e-05, 'test': 'test1'},
                                                                                        {'lr0': 1e-05, 'test': 'test2'}]
...
def test_random_search_params_empty_list():
    ...
    flag = False
    if generated_params == [{'test': 'test2'}, {'test': 'test1'}] or generated_params == [{'test': 'test1'},
                                                                                          {'test': 'test2'}]:
        flag = True
...
def test_random_search_params_invalid_combination_count():
    ...
    flag = False
    true_params = [{'epochs': 2, 'test': 'test2'}, {'epochs': 2, 'test': 'test1'}, {'epochs': 1, 'test': 'test1'},
                   {'epochs': 1, 'test': 'test2'}]
