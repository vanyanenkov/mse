...
function addHyperparamRow() {
  ...
  newRow.innerHTML = `
    <input type="text" name="hyperparam_name[]" placeholder="Название (например: learning_rate)" class="hyperparam-name" />
    <input type="text" name="hyperparam_values[]" placeholder="Значения (например: 0.001 0.01 0.1)" class="hyperparam-values" />
...
function collectHyperparams() {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (name && valuesStr) {
      ...
      hyperparams[name] = valuesArray;
    } else if (name || valuesStr) {
      console.warn(`Гиперпараметр "${name}" имеет незаполненные значения или наоборот`);
    }
  }
