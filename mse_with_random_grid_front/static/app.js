...
function appendTrainingMonitorLog(line) {
  ...
  // Автоматически обновляем статус при добавлении важных сообщений
  if (line.includes('завершено') || line.includes('✅')) {
    setTrainingMonitorStatus('✅ Обучение завершено');
  } else if (line.includes('Ошибка') || line.includes('❌')) {
...
function startTrainingMonitorPolling(runId) {
  ...
  const updateLogs = async () => {
    try {
      ...
      // Проверяем последние строки логов на наличие маркеров завершения
      ...
      if (lastLines.includes('ОБУЧЕНИЕ УСПЕШНО ЗАВЕРШЕНО') || 
        ...
        // Останавливаем polling, так как обучение завершено
        if (state.trainingMonitor.pollingInterval) {
          clearInterval(state.trainingMonitor.pollingInterval);
          state.trainingMonitor.pollingInterval = null;
        }
        
        // Обновляем статус запуска в общем списке
        refreshCoreData();
        
      } else if (lastLines.includes('Ошибка') || lastLines.includes('❌')) {
        setTrainingMonitorStatus('❌ Ошибка обучения');
        ...
        // Обновляем статус из последней строки
...
function addHyperparamRow() {
  ...
  newRow.innerHTML = `
    ...
    <select name="hyperparam_type[]" class="hyperparam-type">
      ...
      <option value="range">Диапазон (мин макс шаг)</option>
    </select>
...
function collectHyperparams() {
  ...
  const types = document.querySelectorAll('select[name="hyperparam_type[]"]');
...
async function renderDatasetsPage() {
  ...
  if (addBtn) {
    addBtn.addEventListener("click", addHyperparamRow);
  }
